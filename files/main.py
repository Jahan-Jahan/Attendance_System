import mediapipe as mp
import cv2 as cv
import numpy as np
import glob
import os
from deepface import DeepFace
from scipy.spatial.distance import cosine
from database_read import get_data
from database_register import save_into_database

class AuthenticationSystem:
    def __init__(self):
        self.threshold = 0.75

    def preprocess_frame(self, frame, face_detection):
        H, W, _ = frame.shape

        x = None
        y = None
        w = None
        h = None

        frame_rgb = cv.cvtColor(frame, cv.COLOR_BGR2RGB)

        out = face_detection.process(frame_rgb)

        detections = out.detections
        if detections is not None:
            ret = True
            for detection in detections:
                location = detection.location_data
                bbox = location.relative_bounding_box

                x, y, w, h = bbox.xmin, bbox.ymin, bbox.width, bbox.height

                x = int(x * W)
                y = int(y * H)
                w = int(w * W)
                h = int(h * H)

                break
        else:
            ret = False
        
        return ret, (x, y, w, h)

    def capture(self, face_detection, screen_name="Register"):
        cap = cv.VideoCapture(0)

        img = None
        x = None
        y = None
        w = None
        h = None

        while True:
            ret, frame = cap.read()

            if not ret:
                break
            
            is_face, (x, y, w, h) = self.preprocess_frame(frame, face_detection)
            frame_copy = frame.copy()
            
            cv.putText(frame_copy, "Click \"ENTER\" to take a picture.", (25, 25), cv.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 255), 2)

            if is_face:
                cv.rectangle(frame_copy, (x, y), (x + w, y + h), (0, 255, 0), 2, cv.LINE_AA)

            cv.imshow(screen_name, frame_copy)

            if cv.waitKey(1) & 0xFF == 13:
                if is_face:
                    img = frame
                    break
                else:
                    print("No face detected!")
        
        cap.release()
        cv.destroyAllWindows()

        return img

    def get_embedding(self, img):
        try:
            embedding = DeepFace.represent(img, model_name="Facenet", enforce_detection=False)
            embedding = np.array(embedding[0]["embedding"])
        except:
            print("There is a problem in face detection.")
            return None
        return embedding

    def get_similarity(self, emb1, emb2):
        return 1 - cosine(emb1, emb2)

    def check(self, name, img=None):
        filepath = os.path.join("people", f"{name}.jpg")
        output = None
        embedding = None

        if os.path.exists(filepath):
            embedding = get_data(name)
        else:
            if img is not None:
                embedding = self.get_embedding(img)
            output = (False, 1)  # Not registered

        if embedding is None:
            output = (False, 3)  # Failure

        sources = {}
        people_path = os.path.join("people", "*.jpg")
        all_people = glob.glob(people_path)

        for p in all_people:
            name_ = os.path.basename(p).replace(".jpg", "")
            sources[name_] = get_data(name_)

        for another_name, source_embedding in sources.items():
            if embedding is None:
                break

            similarity = self.get_similarity(embedding, source_embedding)
            print(another_name, similarity)

            if similarity > self.threshold and name != another_name:
                output = (False, 2)  # Face registered under another name
                break

        return output if output is not None else (True, 0)


    def login(self, name, face_detection):
        source_path = os.path.join("people", f"{name}.jpg")
        if not os.path.exists(source_path):
            return False, 0
        
        response, code = self.check(name)

        if response:
            img = self.capture(face_detection, "Login")
            current_embedding = self.get_embedding(img)
            if current_embedding is None:
                return False, 0

            source_embedding = get_data(name)
            similarity = self.get_similarity(current_embedding, source_embedding)
            
            if similarity > self.threshold:
                return True, similarity
            
            return False, similarity
        else:
            if code == 1:
                print("Not registered.")
            elif code == 2:
                print("This face registered with another name.")
            else:
                print("Failure.")

            return False, 0

    def register(self, face_detection):
        img = self.capture(face_detection)
        name = input("Enter your name: ").strip().lower()
        response, code = self.check(name, img=img)

        if not response:
            if code == 1:
                img_filepath = os.path.join("people", f"{name}.jpg")

                embedding = self.get_embedding(img)
                save_into_database(name, embedding)

                cv.imwrite(img_filepath, img)

                # embedding_filepath = os.path.join("source", f"{name}.npy")
                # np.save(embedding_filepath, embedding)

                print("You registered now!")
            elif code == 2:
                print("This face registered with another name.")
            else:
                print("Failure.")
        else:
            print("Error")

    def start(self, face_detection):
        task = input("Login(L/l) or Register(R/r)?").strip().lower()

        if task in ["login", "l"]:
            name = input("Enter your name: ").strip().lower()

            response, similarity = self.login(name, face_detection)
            if response:
                print("Login successful!")
                # print(f"Confidence: {similarity}")
            else:
                print("You are'n that person or you've been never register before, please do it.")
                ans = input("Register(R/r) or Cancel(C/c)? ").strip().lower()

                if ans in ["register", "r"]:
                    self.register(face_detection)
                else:
                    print("Bye Bye!")
        else:
            self.register(face_detection)

    def main(self):
        print("Authentication system starts.")
        mp_face_detection = mp.solutions.face_detection
        face_detection = mp_face_detection.FaceDetection(model_selection=0, min_detection_confidence=0.5)
        self.start(face_detection)

if __name__ == "__main__":
    app = AuthenticationSystem()
    app.main()